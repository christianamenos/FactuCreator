  ...
  <div class="container">
    ...
    if(filter_has_var(INPUT_POST, 'submit')){
      ...
      if($dataSentAndCorrect){
        ...
        $sql = 'INSERT INTO company (id, name, own, mail, cif, phone, fax, webpage, logo, country, province, city, postal_code, street, number, floor, door, bank_account)
                VALUES (NULL, :name, :own, :mail, :cif, :phone, :fax, :webpage, :logo, :country, :province, :city, :postal_code, :street, :number, :floor, :door, :bank_account)';
        $stmt = $dbh->prepare($sql);
        $res = $stmt->execute(array(':name' => $formValueName,
                             ':own'         => $formValueOwn,
                             ':mail'        => $formValueMail,
                             ':cif'         => $formValueCif,
                             ...
                             ':door'        => $formValueDoor,
                             ':bank_account'=> $formValueBankAccount));
        if($res){
          $insertedDataOk = true;
          $dbh->commit();
        }
    ...
    ?>
      ...
      <div id="contact-form">
        <form method="post" action="new-company.php" enctype="multipart/form-data" class="well">
