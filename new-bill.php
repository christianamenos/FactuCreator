    ...
    <div class="container">
      ...
      if (filter_has_var(INPUT_POST, 'submit')) {
        ...
        if ($dataSentAndCorrect) {
          ...
          $sql = 'INSERT INTO bill (id, sender_id, addressee_id, creation_date, expiration_date, payed, pay_mode, vat_id, reference, language_id)
                VALUES (NULL, :sender_id, :addressee_id, :creation_date, :expiration_date, :payed, :pay_mode, :vat_id, :reference, :language_id)';
          $stmt = $dbh->prepare($sql);
          $res = $stmt->execute(array(':sender_id' => $formValueSender,
              ':addressee_id' => $formValueAddressee,
              ':creation_date' => date('Y-m-d', strtotime('-1 second', strtotime('+0 month', strtotime(date('m') . '/01/' . date('Y') . ' 00:00:00')))),
